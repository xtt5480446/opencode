...
import { disposeAllInstances, provideTmpdirInstance, TestInstance, withTestInstance } from "../fixture/fixture"
...
const createWorkspace = (input: Workspace.CreateInput) => Workspace.Service.use((workspace) => workspace.create(input))
const warpWorkspaceSession = (input: Workspace.SessionWarpInput) =>
  Workspace.Service.use((workspace) => workspace.sessionWarp(input))
const listWorkspaces = (project: Parameters<Workspace.Interface["list"]>[0]) =>
  Workspace.Service.use((workspace) => workspace.list(project))
const syncListWorkspaces = (project: Parameters<Workspace.Interface["syncList"]>[0]) =>
  Workspace.Service.use((workspace) => workspace.syncList(project))
const getWorkspace = (id: WorkspaceID) => Workspace.Service.use((workspace) => workspace.get(id))
const removeWorkspace = (id: WorkspaceID) => Workspace.Service.use((workspace) => workspace.remove(id))
const workspaceStatus = () => Workspace.Service.use((workspace) => workspace.status())
...
const waitForWorkspaceSync = (workspaceID: WorkspaceID, state: Record<string, number>, signal?: AbortSignal) =>
  Workspace.Service.use((workspace) => workspace.waitForSync(workspaceID, state, signal))

const expectEffectFailure = <A, E, R>(effect: Effect.Effect<A, E, R>, message: string) =>
  Effect.gen(function* () {
...
describe("workspace CRUD", () => {
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        expect(yield* getWorkspace(WorkspaceID.ascending("wrk_missing_get"))).toBeUndefined()
      }),
    { git: true },
  )
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(yield* listWorkspaces(instance.project)).toEqual([a, b])
      }),
    { git: true },
  )
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const info = yield* createWorkspace({
          id: workspaceID,
          type,
          branch: null,
        ...
        expect(yield* getWorkspace(workspaceID)).toEqual(info)
        expect(yield* listWorkspaces(instance.project)).toEqual([info])
        ...
        expect((yield* workspaceStatus()).find((item) => item.workspaceID === workspaceID)?.status).toBe("connected")

        yield* removeWorkspace(workspaceID)
        expect((yield* workspaceStatus()).find((item) => item.workspaceID === workspaceID)?.status).toBeUndefined()
      }),
    { git: true },
  )
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(yield* listWorkspaces(instance.project)).toEqual([])
      }),
    { git: true },
  )
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const info = yield* createWorkspace({ type, branch: null, projectID: instance.project.id, extra: null })

        expect(info.directory).toBe(missing)
        expect((yield* workspaceStatus()).find((item) => item.workspaceID === info.id)?.status).toBe("error")
        yield* removeWorkspace(info.id)
      }),
    { git: true },
  )
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* syncListWorkspaces(instance.project)
        const synced = (yield* listWorkspaces(instance.project)).filter((item) => item.name === discovered.name)

        expect(synced).toHaveLength(1)
        expect(synced[0]).toMatchObject(discovered)
        expect(synced[0]?.id).toStartWith("wrk_")
        expect(yield* listWorkspaces(instance.project)).toEqual(expect.arrayContaining([existing, synced[0]]))
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* syncListWorkspaces(instance.project)
        const synced = yield* listWorkspaces(instance.project)
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        expect(yield* removeWorkspace(WorkspaceID.ascending("wrk_missing_remove"))).toBeUndefined()
      }),
    { git: true },
  )
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(yield* removeWorkspace(info.id)).toEqual(info)
        expect(yield* getWorkspace(info.id)).toBeUndefined()
      }),
    { git: true },
  )
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const workspaceDir = path.join(instance.directory, "warp-detach-workspace-instance")
        yield* Effect.promise(() => initGitRepo(workspaceDir))
        ...
        const workspaceCtx = yield* Effect.promise(() =>
          withTestInstance({ directory: workspaceDir, fn: (ctx) => ctx }),
        )
        const workspaceProjectID = workspaceCtx.project.id
        expect(workspaceProjectID).not.toBe(projectID)
        yield* warpWorkspaceSession({ workspaceID: null, sessionID: session.id }).pipe(
          Effect.provideService(InstanceRef, workspaceCtx),
        )
...
describe("workspace waitForSync", () => {
  it.instance("returns immediately for an empty fence", () =>
    waitForWorkspaceSync(WorkspaceID.ascending("wrk_wait_empty"), {}),
  )

  it.instance("returns immediately when the stored sequence already satisfies the fence", () =>
    Effect.gen(function* () {
      ...
      yield* waitForWorkspaceSync(WorkspaceID.ascending("wrk_wait_done"), { [sessionID]: 4 })
      yield* waitForWorkspaceSync(WorkspaceID.ascending("wrk_wait_done_2"), { [sessionID]: 3 })
    }),
  )

  it.instance("waits until the database reaches the requested sequence and a workspace event arrives", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.all(
        [
          waitForWorkspaceSync(workspaceID, { [sessionID]: 2 }),
          Effect.gen(function* () {
            yield* Effect.sleep("10 millis")
            Database.use((db) =>
  ...
  it.instance("a sync event for a different workspace can also release the fence", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.all(
        [
          waitForWorkspaceSync(workspaceID, { [sessionID]: 1 }),
          Effect.gen(function* () {
            yield* Effect.sleep("10 millis")
            Database.use((db) =>
  ...
  it.instance("rejects with the abort reason when aborted", () =>
    Effect.gen(function* () {
      ...
      const exit = yield* Effect.exit(
        Effect.all(
          [
            ...
            Effect.sync(() => abort.abort(reason)),
          ],
          { concurrency: 2 },
        ),
      )
      ...
      expect(Exit.isFailure(exit)).toBe(true)
      if (Exit.isFailure(exit)) expect(Cause.pretty(exit.cause)).toContain(reason.message)
    }),
  )
