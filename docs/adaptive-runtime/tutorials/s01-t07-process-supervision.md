...
terminal path 由单一不可中断 owner 执行：process-group kill、exit observation、pending RPC interruption、stderr reader、Store settlement 和 Queue/PubSub shutdown 都有独立 deadline。并发的 stop、lease watcher、stdout reader、process exit watcher 或 scope finalizer 只会等待同一个预创建 latch。已有 exit code、kill 后 `isRunning === false`，或成功读取 exit code 都可以确认进程已经终止；若 exit observation defect/timeout 且 kill/process termination 仍未确认，ownership 必须保持 uncertain。kill/group cleanup 或 RPC interruption 失败/超时也会保持 uncertain，即使随后读到了 exit code。只有 cleanup 得到确认后，`settleAgent()` 才会清空 owner/PID；已经确认 group 消失但无法再读取 exit code 时可用 `128` 安全结算。uncertain path 会改走 exact `quarantineAgent()` CAS，把 Agent 记为 `failed`、保留 owner/PID、把 lease 置空并持久化 cleanup diagnostic。这个 tuple 不可过期也不可 claim，fresh supervisor 会在 spawn 前拒绝；当前 supervisor 同时保留 active tombstone。quarantine 自身失败也会保留 tombstone 并暴露 `TerminationError`，不会伪装成功。普通 Store settlement 与 quarantine 都最多尝试三次，scope cleanup 仍然有界。
...
| cleanup 不确定却释放 ownership               | 注入 kill defect、未确认终止时的 exit defect/timeout、RPC interruption timeout 和 quarantine CAS failure | owner/PID 保留、lease null、generation 不变、diagnostic 精确；same/fresh supervisor 都不能重启   | uncertain cleanup fail closed，scope cleanup 仍有界                        |
...
预期观察：focused supervisor 文件的 41 个 case 全部通过，四个 startup failure case 在返回前完成 cleanup，invalid hidden subprocess 都为 exit `64`，uncertain cleanup（包括未确认终止时的 exit observation defect/timeout）保持 quarantine ownership，ready renewal 与 watcher deadline 对齐，RPC replay/flood 被有界拒绝，stderr preview 在 terminal completion 前冻结，silent/mixed-group case 结束后 PID 与 PGID 都不存在。测试数量是当前实现的运行观测，未来增加 case 时应按实际输出更新。
