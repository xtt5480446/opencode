...
S01-T08 新增了 `AdaptiveModelGateway`：Adaptive Agent 只能提交 Task、Agent generation、Manifest 与 Request 的 durable ID，网关再从 Store 读取冻结的 `ModelPolicy` 和完整 `ContextManifest`，通过 OpenCode 现有模型解析与 credential 路径发起一次 `LLMClient.stream`。请求从 `admitted` 进入 `streaming`；只有真正观察到 canonical `finish` 才能结算为 `succeeded`，provider 失败、提前停止与调用方中断分别结算为 `failed|interrupted`。若 terminal settlement 自身失败，调用方会收到不含底层 secret 的 `SettlementError`，而不是被伪装成一次成功调用。
...
第二段是 canonical LLM transport：网关使用 `LLM.request` 构造请求，再调用一次 `LLMClient.Service.stream`。上层仍消费 OpenCode/Core 已有的统一 `LLMEvent` 协议；网关只在信任边界重建新的 canonical event，保留 event type、block/tool ID、step index、finish reason、normalized usage 等关联事实，并移除 provider 私有 metadata 与原始错误载荷。它没有另造 Adaptive 私有事件协议。
...
StreamInput IDs
  ...
  → observe raw terminal/usage facts
  → sanitize into new canonical public events
...
`RequestState` 只保存结算需要的少量事实：policy、resolved provider、effective limit、最新的 input/output token counters、provider 是否开始、是否收到 `provider-error`，以及是否真正观察到 canonical `finish`。不同 usage event 可以分别提供 input 或 output，网关会保留每个字段最近一次有效的非负整数值，不会因后续事件省略另一个字段而清空已观测计数。审计观察发生在公开脱敏之前，因此数据库仍能得到准确的 normalized usage 和 terminal 事实。

Scope finalizer 根据 stream exit 和已观察事件共同决定 terminal status：scope 正常结束且收到 canonical `finish` 才是 `succeeded`；canonical `provider-error` event、transport failure、下游通过 `runHead` 等方式提前停止，或 provider 在没有 `finish` 时自然 EOF，都是 `failed`；fiber interruption 是 `interrupted`。提前结束统一保存固定摘要 `Provider stream ended before finish`，不会把“流没报错”误判成“模型完成了回答”。

credential-free 调用方不会收到可能带 provider 或 credential 细节的底层 cause。resolver/location 的 typed failure、`Integration.AuthorizationError.cause` 和 defect 统一映射成只含 `requestID + "Model resolution failed"` 的 `ModelResolutionError`；Manifest load corruption 与 Message/Tool decode failure 统一映射成只含 IDs 和固定 reason 的 `InvalidManifestContentError`。transport 的原始 `LLMError`，以及 `llm.stream(request)` 在返回 Stream 前同步抛出的 defect，都转换成不带 cause/message 的 `ProviderStreamError`，且 provider stream 只构造一次。

公开 event 也执行同一条信任边界规则：每个 raw event 都被重建成新对象，删除顶层 `providerMetadata` 和 `usage.providerMetadata`；`provider-error` 只保留安全的 `classification/retryable` 并使用固定 message，`tool-error` 只保留 `id/name` 并使用固定 message，原始 `error` 不会跨过边界。Terminal settlement 在不可中断 finalizer 中最多尝试三次，不做无界等待。瞬时数据库/审计 defect 会在有限重试内恢复；若仍失败，网关先通过同一 Request identity 的 CAS recovery path 写入固定 `failed` 终态 `Model request settlement failed`，再向调用方抛只含 `requestID` 的 `SettlementError`。这种调用结果仍然是评测无效的，`AdaptiveModelAudit.verify` 会报告 `SETTLEMENT_RECOVERY_REQUIRED:<requestID>`，而不是把 provider 结果算作有效或留下 `UNSETTLED_MODEL_REQUEST`。如果连 recovery path 也不可用，安全错误仍会暴露给上层，外部 durable recovery/运维队列必须接管；实现不会伪造成功。
...
3. 阅读 `RequestState`、`observeRawEvent()`、`sanitizeEvent()` 与 `settlement()`，理解 raw facts 怎样先用于审计、再以脱敏 canonical event 公开，以及成功、提前结束、provider failure 和 interruption 怎样统一落库。
...
Canonical event 是 `@opencode-ai/llm` 对不同 provider 流事件的统一表示，例如 `text-delta`、`step-finish`、`finish` 和 `provider-error`。网关返回同一 schema 下的新 event 对象，保留跨事件关联所需的 ID、index、reason 和 normalized usage，但不转发 provider 私有 metadata、原始 provider/tool error；因此既兼容现有消费者，也不创建 Adaptive 私有 token-stream 协议。
...
| canonical provider error 被误判成功或泄密          | `settles a provider-error event`                                        | 返回新的固定-message event；row 为 redacted `failed`                                              | canonical correlation 与 audit status 同时正确               |
...
| 下游提前停止或 provider 无 `finish` EOF 被误判成功 | 两个 `does not succeed` 测试                                            | row 为 `failed`，固定摘要为 `Provider stream ended before finish`                                 | success 必须由 canonical terminal event 证明                 |
| raw event metadata/error 穿过 child 边界           | `sanitizes provider, tool, and usage metadata`                          | event ID/index/usage 保留；所有 provider metadata 与原始 error message 消失                       | public stream 不泄露 provider 私有载荷                       |
| settlement defect 被吞掉并返回假成功               | `fails the caller when terminal audit settlement fails`                 | safe `SettlementError`；row 为 fixed `failed`；verify 报 `SETTLEMENT_RECOVERY_REQUIRED`           | 审计结算失败可见、无未完成 row 且结果无效                    |
| 瞬时 settlement defect 未被有限重试                | `retries one transient settlement defect`                               | 第一次 defect 后第二次成功；provider 结果正常返回；attempts 精确为 2                              | 短暂数据库/审计故障可恢复且不重复 provider call              |
| provider Stream 构造时同步 defect 泄密或重复调用   | `sanitizes synchronous defects while constructing the provider stream`  | safe `ProviderStreamError`；secret 不可见；`LLMClient.stream` 调用次数精确为 1                    | Stream 返回前的同步异常同样受网关保护                        |
