...
公开 event 也执行同一条信任边界规则：每个 raw event 都被重建成新对象，删除顶层 `providerMetadata` 和 `usage.providerMetadata`；`provider-error` 只保留安全的 `classification/retryable` 并使用固定 message，`tool-error` 只保留 `id/name` 并使用固定 message，原始 `error` 不会跨过边界。若最终 `audit.settle` 失败，uninterruptible finalizer 以只含 `requestID` 和固定 reason 的 `SettlementError` 让整个调用失败；它不会吞掉异常或宣称成功。此时 durable request 保持 `streaming`，后续 `AdaptiveModelAudit.verify` 会明确报告 `UNSETTLED_MODEL_REQUEST`。
...
| settlement defect 被吞掉并返回假成功               | `fails the caller when terminal audit settlement fails`                 | safe `SettlementError`；row 仍为 `streaming`；verify 报 `UNSETTLED_MODEL_REQUEST`                 | 审计结算失败可见且 fail-closed                               |
