...
Request 保存完整 ModelPolicy 字段加 hash，而不只保存 `modelID`。这样未来 benchmark audit 能独立重算每次请求使用的 provider、variant 和 context budget。插入前会重算 policy hash；malformed/stale snapshot 返回 `InvalidRequest`，不会越过公开错误通道成为进程 defect。
...
当前预期是 migration test `17 pass / 0 fail`，并看到：
