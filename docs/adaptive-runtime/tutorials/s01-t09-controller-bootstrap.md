...
本任务第一次把 Stage 1 已完成的 Task、ModelPolicy、进程监督和 Model Gateway 串成真实 CLI 流程。用户可以显式运行 `opencode run --runtime adaptive`：系统先持久化 Task，再启动一个没有 provider credential 的 Coordinator 子进程，由子进程经 Controller 发起恰好一次模型请求，最后留下可由新 CLI 进程读取和导出的完整审计记录。
...
为让真实 subprocess 测试使用 OpenCode 已有的 inline config，Core V2 `Config` 现在把 `OPENCODE_CONFIG_CONTENT` 解码成最高优先级 `Config.Document`；`Catalog` 也把 AISDK `api.settings.apiKey` 识别为可用 credential。这与 `SessionRunnerModel` 原本从同一 settings 读取 key 的行为对齐，文件配置的发现顺序保持不变。

## 最终实现

`RunCommand` 新增 `--runtime baseline|adaptive`，默认值是 `baseline`。Adaptive 分支先拒绝 `--continue`、`--session`、`--fork`、`--command`、`--share`、`--attach` 和 `--interactive`，避免用户以为旧 Session 选项仍然生效；然后按 baseline 规则解析 `--dir` 和 piped stdin。不存在的目录直接失败，不会先创建 Task。
...
Coordinator 的入口是 `runAdaptiveRole()`。它不加载 AppRuntime，也不自行选择模型；coordinator role 只调用一次 `context.modelStream(null)`，收集 `text-delta`，再提交结构化 `{ type: "bootstrap.completed", bootstrap }`。Controller 对第二次 model request、重复 completion、非法 completion payload 和非 JSON 模型事件全部 fail closed。模型事件进入 NDJSON 协议前会移除 `undefined`，usage 只保留标准 token 字段；provider 私有 metadata 不会穿过 child 边界。

用户可观察接口包括：

- 默认输出中 Task ID 恰好出现一次；`--format json` 的第一行一定是 `adaptive.task.created`，后续事件携带同一 Task ID。
- `adaptive status <task-id> --json` 从 SQLite 返回 Task、完整 ModelPolicy、最新 process identity/state 和 request lineage/usage；它不依赖创建 Task 的旧进程。
- `adaptive doctor --offline --json` 检查四张 foundation table、audit columns、child command、协议 round-trip 和工作区写权限，不调用 provider。
...
- `adaptive export --doctor doctor.json --output evidence` 只接受与当前 SQLite 权威状态一致的 live doctor JSON。它输出真实 `doctor.json`、`model-requests.jsonl`、`process.json` 和覆盖前三个文件的 `SHA256SUMS`；输出目录或文件已存在时拒绝覆盖。
...
cd ../opencode
bun test test/adaptive/controller.test.ts test/adaptive/process-supervisor.test.ts
...
T09 没有实现 Roadmap、Detail Pool、Context Assembler、worker coding tools、多 Agent 调度或上下文重建。它只建立这些能力将来必须经过的可靠入口：durable Task、独立进程、generation-bound Manifest、单模型 Gateway 和可导出审计。当前 Task 保持 `planning`，bootstrap 文本只是 Coordinator 的第一条受控结果。
