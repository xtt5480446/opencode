...
为让真实 subprocess 测试使用 OpenCode 已有的 inline config，Core V2 `Config` 现在把 `OPENCODE_CONFIG_CONTENT` 解码成最高优先级 `Config.Document`。baseline 已验证的 `ConfigVariable` 被下沉到 Core，旧路径只做 re-export，因此 `{env:...}` 和 `{file:...}` 不会出现两套规则；inline 的相对文件以当前 `Location.directory` 为基准。`Catalog` 除了识别 request body 和 AISDK settings 中的 `apiKey`，还只把非空的 `Authorization`、`x-api-key`、`x-goog-api-key`、`api-key` 识别为 credential，普通 header 不会放行 provider。这些改动复用原有配置语义，文件发现顺序保持不变。
...
| credential header 被漏判或泛化      | 配置四种标准 key header、普通 header 和空 key              | 只返回四个非空 credential provider                                      | V1 lowerer 结果可用且普通业务 header 不会冒充凭据           |
