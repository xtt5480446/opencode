...
为让真实 subprocess 测试使用 OpenCode 已有的 inline config，Core V2 `Config` 现在把 `OPENCODE_CONFIG_CONTENT` 解码成最高优先级 `Config.Document`。baseline 已验证的 `ConfigVariable` 被下沉到 Core，旧路径只做 re-export，因此 `{env:...}` 和 `{file:...}` 不会出现两套规则；inline 的相对文件以当前 `Location.directory` 为基准。`Catalog` 只接受非空 request body/AISDK settings `apiKey`，以及包含真实 credential 内容的 `Authorization`、`x-api-key`、`x-goog-api-key`、`api-key`；空字符串、只有 `Bearer` scheme 的 header 和普通业务 header 都不会放行 provider。这些改动复用原有配置语义，文件发现顺序保持不变。

## 最终实现

`RunCommand` 新增 `--runtime baseline|adaptive`，默认值是 `baseline`。Adaptive 分支先拒绝 `--continue`、`--session`、`--fork`、`--command`、`--share`、`--attach`、`--interactive` 和 `--file`，再要求 `--model` 存在且符合 `provider/model`；这些校验都发生在读取 piped stdin 之前。通过校验后才按 baseline 规则解析 `--dir` 和 stdin。不存在的目录直接失败，不会先创建 Task；非空 requirement 只用 `trim()` 判断是否为空，写入 Task 和 Manifest 的仍是用户原文，缩进、换行和首尾空白不会被改写。

一次成功调用的控制流如下：

```text
opencode run --runtime adaptive --model provider/model "requirement"
  → validate incompatible CLI options and exact model shape
  → resolve directory and stdin; preserve the original nonblank requirement
  → wait for final catalog-ready plugin after external/config/variant loading
  → reload Integration and Catalog
...
Coordinator 的入口是 `runAdaptiveRole()`。它不加载 AppRuntime，也不自行选择模型；coordinator role 只调用一次 `context.modelStream(null)`，收集 `text-delta`，再提交结构化 `{ type: "bootstrap.completed", bootstrap }`。Controller 不会仅凭这条 RPC 宣布成功：它先 trim 并拒绝空白 bootstrap，随后 `AdaptiveStore.completeBootstrap()` 必须在同一事务中确认 Task/Agent/generation/Manifest/Request 完全一致、Agent role 是 Coordinator、Request 已经 `succeeded`，同时 Agent 仍由该 generation 的非空 owner 持有且 lease 未过期，才写入唯一的 `adaptive_bootstrap`。旧 generation 即使已经拿到成功 Request，也不能在 replacement controller 接管后补写结果。没有 request、自然 EOF、provider error、空白模型结果、failed request、stale generation 或重复 completion 都会 fail closed。模型事件进入 NDJSON 协议前会移除 `undefined`，usage 只保留标准 token 字段；provider 私有 metadata 不会穿过 child 边界。

Controller 必须等 external/config/generated variant 插件全部完成后再解析精确模型，否则用户配置的 provider 可能被误报为不存在；但外部插件也可能永久卡在安装或 import。生产等待因此由 `CATALOG_READY_TIMEOUT_MS` 限制为 5 分钟，给首次 npm 安装和较慢的私有 registry 留出空间；超时返回 `AdaptiveController.CatalogUnavailable`，并且发生在 Task 创建和 child 启动之前。这样既不跳过合法插件定义，也不会让 `run --runtime adaptive` 或 live doctor 无限挂起；用户仍可随时用 Ctrl-C 取消等待，`PluginInternal` 本身仍在后台启动，baseline 路径没有被改成同步等待这条 barrier。
...
- `adaptive doctor --offline --json` 检查五张 foundation table、audit columns、工作区写权限，并真实启动无模型 discovery child，完成 `hello → accepted → ready → shutdown`。它因此能发现 packaged binary 缺少 hidden entry 或无法调度 child，而不是只证明 command object 能构造；整个过程不调用 provider。
...
2. 再读 Core `config/variable.ts`、V2 `Config` 的 `loadText()` 和 `Catalog.provider.available()`，理解 baseline 配置语义如何被 Adaptive 复用。
3. 阅读 `AdaptiveController.start()`，按模型解析、Task、Agent、`prepare()` Manifest、RPC router 和 completion 顺序跟一遍主链。
4. 阅读 `runAdaptiveRole()`，确认 child 的业务逻辑只有一次 `model.stream` 和一次 `process.complete`，没有 provider/config 访问。
5. 阅读 `AdaptiveProcessSupervisor.StartInput.prepare` 及 `start()` 中 claim/prepare/accepted 的相对位置。
6. 阅读 `AdaptiveCommand` 中 `foundationChecks()`、`taskSummary()`、live doctor 和 export，理解运维命令如何只读 Store 或复用 Controller。
7. 最后阅读 `adaptive-process.test.ts`，观察测试如何通过真实 CLI subprocess、fake HTTP provider 和第二个 CLI 进程验证持久状态。
...
| requirement 被 trim 后无法恢复       | 传入带首尾空白和多行缩进的需求，再查询 Task 与 Manifest       | 两处文本逐字符等于 CLI 原始参数                                         | durable state 能重建模型真正看到的原始需求                  |
...
| 旧 generation 越权补写 bootstrap     | lease 过期后由 replacement owner claim，再提交旧结果          | 返回 `BootstrapReferenceMismatch` 且 Store 中不存在 bootstrap           | ownership 变更后旧 Controller 无法污染恢复状态              |
...
| catalog 尚未完整加载就解析模型       | external plugin 返回后立即读状态；Controller 记录等待 ID      | external state 已可见，且等待最终 `variant` barrier                     | external/config/generated variant 都先于 exact resolution   |
| external plugin 永久不 ready         | fake plugin 的 `wait()` 永不返回，测试注入 1ms readiness 上限 | 返回 `AdaptiveController.CatalogUnavailable`，后续 reload 不会执行      | 生产等待有 5 分钟上限，超时前不创建 Task 或 child           |
| offline doctor 只检查 command object | 用 wrapper 让主 CLI 正常但 hidden child 明确退出              | doctor 非零退出并报告 process unavailable                               | 本地自检真实覆盖 hidden entry dispatch 和协议握手           |
...
| inline config token 未展开           | 分别在 inline content 使用 `{env:...}` 与相对 `{file:...}`    | env 值正确；文件从 active Location 读取并 trim                          | Adaptive 模型配置沿用 baseline 的 credential 引用语义       |
| credential header 被漏判或泛化       | 配置四种标准 key header、普通 header、空 API key 和空 Bearer  | 只返回包含真实 credential 内容的 provider                               | V1 lowerer 结果可用且空替换值或业务 header 不会冒充凭据     |
| 无效参数等待永不结束的 stdin         | 保持 stdin pipe 打开并传 `--session`                          | 两秒内返回 incompatibility                                              | compatibility gate 先于 stdin drain                         |
| 缺失或畸形 model 等待 stdin          | 保持 stdin pipe 打开并省略 `--model` 或只传 provider          | 两秒内返回明确的 `provider/model` 参数错误                              | 模型参数 gate 先于 stdin drain                              |
...
预期观察：Core Store/Config/Catalog 测试全部通过，其中包括 stale generation、inline token 和 credential header 边界；Adaptive subprocess 文件包含 offline doctor、runtime isolation、原始 requirement 保真、model fail-fast、`--dir`、audited request、live doctor 和 export 用例；baseline runtime parity 用例通过；两个 package 的 typecheck 为 exit `0`。测试会自己启动本地 fake HTTP provider，不需要真实 API key。
