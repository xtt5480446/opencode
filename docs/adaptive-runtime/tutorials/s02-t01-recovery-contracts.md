...
- 沿用 OpenCode 的 branded path 表达方式，但为权限边界新增带真实校验的 `RepositoryPath` 和 `RepositoryGlob`。
...
现有 `RelativePath` 也不能直接承担 Worker mutation allowlist：它只是 TypeScript brand，解码时不会拒绝 `/tmp/file` 或 `../secret`。本任务的 repository path/glob 会拒绝绝对路径、Windows drive/backslash、控制字符、空 segment、`.` 和 `..`，并统一使用 `/`，从契约入口阻止路径逃逸。`RepositoryPath` 还会拒绝 glob 元字符，确保 Checkpoint 和 key-file hash 指向一个具体文件；只有 `RepositoryGlob` 可以表达 `src/**`。
...
`AdaptiveOperation.Assignment` 是不可变工作单，固定 Task、Worker、node、Roadmap revision、精确 Detail refs、经过校验的 repository-relative path globs、base commit、验收命令和 Agent generation。Checkpoint、RecoveryVerification 和 CandidateReport 中的实际文件则使用非 glob 的 `RepositoryPath`。
...
事件 payload 携带重建投影需要的完整业务值，而不是只保存一段描述文字。例如 `CheckpointSaved` 包含完整 `Checkpoint`；`RoadmapCommitted` 除完整 Roadmap 和 content hash 外，还包含同一事务新增的每个 `DetailRecord` 的 body/hash。这样 S02-T03 删除 projection 后，不需要查询已经删除的 Detail 表也能从事件恢复。

Tool input/output 使用统一 `ToolPayload`，canonical hash 和最多 8192 字符的 preview 都是必填项。`complete=true` 表示 preview 就是完整 canonical payload；一旦 preview 被截断，`complete=false` 会强制要求 content-addressed blob，而且 blob hash 必须等于 canonical hash。后续 T06 可以用 input hash 区分同一 call ID 的 exact retry 与 conflicting reuse，并确信从 blob 读取的是同一份已审计内容。
...
**Bounded preview**：Tool 大输出不会直接塞入事件或后续 prompt，只保留固定上限的可读片段；canonical hash 永远存在，截断时完整内容必须用 content-addressed blob 保存。这防止日志挤出上下文预算，同时保留 exact retry 和结果重放依据。
...
| Roadmap event 无法重建新 Detail              | 解码带 `DetailRecord[]` 的 commit | Detail body/ref/hash 完整保留                                               | 证明 replay 输入完整，不证明 T03 projector 已实现      |
| Tool 日志无限增长或失去 exact identity       | 解码 ToolPayload 边界             | 必须有 hash/preview；截断无 blob、blob/hash 不同或 8193 字符 preview 均失败 | 证明 durable payload 可判等，不证明 T06 已拒绝冲突调用 |
| mutation allowlist 路径逃逸或实际路径含糊    | 解码绝对路径、`..` 和 glob path   | 两类契约拒绝逃逸；`RepositoryPath` 额外拒绝 `src/**`                        | 证明 wire 边界，不替代 T06 的运行时目录检查            |
...
预期观察：16 个聚焦测试全部通过，typecheck 退出码为 0。`adaptive-contract.test.ts` 应展示完整 Roadmap/Assignment/Checkpoint 用法；`event-manifest.test.ts` 应遍历 14 个 Adaptive definitions，而不是手工只检查其中几个。
...
预期观察：33 个测试通过、0 失败。若 manifest count 或 Session/V1 相邻顺序失败，优先检查 `event-manifest.ts` 的注册位置；若 repository path 解码失败，应先检查输入是否为规范化的仓库相对路径，不能把生产契约放宽为普通字符串。
