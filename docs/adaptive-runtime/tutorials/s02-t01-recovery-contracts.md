...
现有 `RelativePath` 也不能直接承担 Worker mutation allowlist：它只是 TypeScript brand，解码时不会拒绝 `/tmp/file` 或 `../secret`。本任务的 repository path/glob 会拒绝绝对路径、Windows drive/backslash、控制字符、空 segment、`.` 和 `..`，并统一使用 `/`，从契约入口阻止路径逃逸。
...
Tool input/output 使用统一 `ToolPayload`，canonical hash 和最多 8192 字符的 preview 都是必填项。`complete=true` 表示 preview 就是完整 canonical payload；一旦 preview 被截断，`complete=false` 会强制要求 content-addressed blob hash。后续 T06 可以用 input hash 区分同一 call ID 的 exact retry 与 conflicting reuse，并用完整 preview 或 blob 重放已结算结果。
...
| Tool 日志无限增长或失去 exact identity       | 解码 ToolPayload 边界              | 必须有 hash/preview；截断无 blob 或 8193 字符 preview 均失败           | 证明 durable payload 可判等，不证明 T06 已拒绝冲突调用 |
| mutation allowlist 路径逃逸                  | 解码绝对路径、drive、反斜杠和 `..` | `RepositoryPath/RepositoryGlob` 全部拒绝                               | 证明 wire 边界，不替代 T06 的运行时目录检查            |
