...
真实 G1 试跑进一步暴露了 OpenCode 过渡期内的两条模型路径：baseline Provider 会读取 `providers login` 写入的 `auth.json`，V2 Catalog/Integration 则只把 V2 credential、环境变量或配置内 credential 视为连接。此前 Adaptive 直接使用 V2 resolver，因此同一个 Kimi 模型在 baseline 可调用，在 live doctor 中却被判定为 unavailable。修复没有另造登录命令或复制 key 到 V2 数据库，而是在 Adaptive 的模型解析边界增加只读兼容：先走 V2；仅当 V2 报模型不可用时，才读取现有 Auth 中同 provider 的 API key，在 Controller 内存里构造同一 V2 route。
...
真实模型解析由 `AdaptiveModelResolver.resolveRef()` 统一承担，Controller 创建不可变 `ModelPolicy` 与 Model Gateway 发起实际请求时都调用它，避免两个阶段各自理解一套 credential 规则。正常 V2 connection 始终优先；legacy fallback 只接受 `providers login` 保存的 API-key auth，并继续检查 provider 未禁用、model 已启用和请求 variant 有效。key 只闭包在 provider route 的认证逻辑中，不写入 Task、Manifest、审计记录或 child 环境。
...
6. 阅读 `packages/opencode/src/adaptive/model-resolver.ts`，理解 V2-first、legacy API-key fallback 和 disabled provider 防绕过逻辑。
7. 最后看 `packages/opencode/test/adaptive/model-resolver.test.ts`、`packages/opencode/test/cli/adaptive-process.test.ts`，以及 G1 acceptance ledger 中仍由用户控制的 Result。
...
**Credential bridge** 不是复制或迁移 secret，而是让新执行路径在需要时读取旧路径已经持有的 credential。本任务只桥接 API-key auth：模型身份、上下文上限和 variant 仍来自 V2 Catalog，key 只用于构造请求认证。这样 OpenCode 现有 `providers login` 对用户仍是单一入口，同时避免两份 credential 存储发生轮换漂移。
...
| baseline 能用但 Adaptive 不认登录 | 模拟 `providers login` auth      | config 不含 key 时 live doctor 仍成功且输出不含 secret          | legacy API key 可被同一模型链复用 |
| credential fallback 绕过策略      | 单元测试 disabled provider       | 即使 Auth 有 key 仍返回原 model unavailable                     | 兼容层不放宽 provider/model 策略  |
...
bun test test/adaptive/model-resolver.test.ts
bun test test/cli/adaptive-process.test.ts --test-name-pattern "release build gates"
```

预期分别看到 `10 pass / 0 fail`、`3 pass / 0 fail` 和 `1 pass / 0 fail`。随后构建当前平台产物：
...
本次兼容只覆盖 legacy API-key auth，正好覆盖 G1 选择的 `Kimi For Coding`。OAuth 的刷新、provider 专用 token 语义和 legacy-to-V2 credential 生命周期不能靠伪造通用 bridge 处理；在对应 provider 纳入商业支持矩阵时，应复用其正式 V2 Integration，或设计有明确刷新所有权的迁移，而不是把 access token 当普通 API key。
